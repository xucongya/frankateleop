...
@hydra.main(config_name="launch_robot_right")
def main(cfg):
    ...
    if cfg.use_real_time:
        ...
        def cleanup():
            ...
            subprocess.check_call(["sudo", "bash", "-c", f"lsof -t -i:{port} | xargs -r kill -9"])

    else:

        def cleanup():
            log.info(f"Killing subprocess with pid {server_output.pid}, pgid {pgid}...")
            subprocess.check_call(["bash", "-c", f"lsof -t -i:{port} | xargs -r kill -9"])

    atexit.register(cleanup)
    signal.signal(signal.SIGTERM, lambda signal_number, stack_frame: cleanup())
    signal.signal(signal.SIGINT, lambda signal_number, stack_frame: cleanup())
